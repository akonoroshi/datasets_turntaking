    ...
    def crop_face_detectron_batchver(self, final_shape, image):
        ...
        for b in range(num_batches):
            ...
            for i in range(data_len):
                ...
                images_per_batch.append({'image':image[b][i].permute(2, 0, 1).type(torch.cuda.FloatTensor)})
        ...
        for outer_b in range(num_batches):
            ...
            for b in range(len(detected_faces_list)):
                ...
                for i in range(len(detected_faces_list[b])):
                    ...
                    image_to_crop = updated_images_list[b][i]['image'].permute(1,2,0).cpu().detach().numpy().astype(np.uint8)
