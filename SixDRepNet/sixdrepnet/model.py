...
class SixDRepNet(nn.Module):
    ...
    def forward(self, x, return_features=True):
        ...
        out5 = out5.to('cpu')
        del out5
        out7 = torch.flatten(out6, 1)
        out6 = out6.to('cpu')
        del out6
        final_out = self.linear_reg(out7)
        out7 = out7.to('cpu')
        del out7
        ...
        if self.gpu_id ==-1:
            output = utils.compute_rotation_matrix_from_ortho6d(final_out, False, self.gpu_id)
            # print(output)
        else:
            output = utils.compute_rotation_matrix_from_ortho6d(final_out, True, self.gpu_id)
            # print(out)
        return output
